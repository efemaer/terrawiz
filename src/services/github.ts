...
export class GitHubService {
  ...
  constructor(options: GitHubServiceOptions = {}) {
    ...
    if (options.repoPattern) {
      try {
        ...
        this.logger.errorWithStack(
          `Invalid repository regex pattern: ${options.repoPattern}`,
          error as Error
        );
    ...
    this.logger.debug(
      `Debug mode: ${isDebugMode}, Skip archived: ${this.skipArchived}, Repo pattern: ${options.repoPattern || 'none'}, IaC file types: ${this.iacFileTypes.join(', ')}`
    );
  }
  ...
  async getIaCFilesFromRepo(repoInfo: RepositoryInfo): Promise<IacFile[]> {
    try {
      ...
      const iacFiles: { path: string; url: string; sha: string; type: IacFileType }[] =
        tree.data.tree
          .filter((item: GitHubTreeItem) => {
            ...
            const isTerragrunt =
              this.iacFileTypes.includes('terragrunt') && isTerragruntFile(item.path);
      ...
      for (const file of iacFiles) {
        try {
          ...
          if (processedCount % 10 === 0 || processedCount === totalFiles) {
            this.logger.info(
              `Processing file ${processedCount}/${totalFiles} in ${repoInfo.fullName}`
            );
          }
          ...
          this.logger.errorWithStack(
            `Error getting content for ${file.path} in ${repoInfo.fullName}`,
            error as Error
          );
        }
      }
      ...
      this.logger.errorWithStack(
        `Error getting IaC files from ${repoInfo.fullName}`,
        error as Error
      );
      return [];
    }
  ...
  async findIacFiles(
    ...
    try {
      ...
      for (const repository of repositories) {
        processedCount++;
        this.logger.info(
          `Processing repository ${processedCount}/${repositories.length}: ${repository.fullName}`
        );
      ...
      this.logger.errorWithStack(
        'Error searching for Infrastructure as Code files',
        error as Error
      );
      throw error;
    }
  ...
  private async getFileContent(owner: string, repo: string, path: string): Promise<string> {
    try {
      ...
      this.logger.errorWithStack(
        `Error getting file content for ${path} in ${owner}/${repo}`,
        error as Error
      );
      throw error;
    }
